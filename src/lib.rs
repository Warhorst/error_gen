...
use syn::{AttributeArgs, Fields, Fields::*, Index, ItemStruct, NestedMeta, parse_macro_input, Generics};
...
fn create_display_implementation(display_string_opt: Option<String>, ident: &Ident, fields: &Fields, generics: &Generics) -> TokenStream2 {
    ...
    let write_parameters = match fields {
        ...
        Unnamed(_) => create_unnamed_write_parameters(&mut display_string, fields),
        Unit => vec![]
    };
...
fn create_unnamed_write_parameters(display_string: &mut String, fields: &Fields) -> Vec<TokenStream2> {
    let mut parameters = vec![];
